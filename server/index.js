...
app.post('/check', async (req, res) => {
  try {
    ...
    // Decode the token
    const decoded = verifyRS256Token(token);
    if (!decoded || !decoded.accountNumber) {
      return res.status(401).json({ valid: false, error: 'Invalid or expired token.' });
    }

    const { accountNumber } = decoded;
    logToFile(`CHECK called for account: ${accountNumber}`);
    ...
    // Optionally fetch bank details if needed
    const bank = await db.collection('banks').findOne({ _id: new ObjectId(account.bankId) });
    if (!bank) {
      return res.status(404).json({ valid: false, error: 'Bank info not found.' });
    }

    // 🔐 Re-encode a fresh token with updated time
    const newToken = generateRS256Token({
      valid: true,
      type: 'check',
      accountNumber: account.accountNumber,
      accountHolderName: account.accountHolderName,
